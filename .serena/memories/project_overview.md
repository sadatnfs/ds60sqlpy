...
- Learner entry point: `README.md`; human navigation: `docs/curriculum-map.md`; generated machine inventory: `curriculum/catalog.json` (regenerate with `.venv/bin/python scripts/build_catalog.py` or `\.venv\Scripts\python.exe scripts\build_catalog.py`).
...
- Course tooling: importable package under `src/ds60sqlpy/`; source-checkout CLI `python scripts/course.py` provides doctor, catalog, validation, progress, and PostgreSQL runners. `bridge/scripts/validate_bridge.py` adds bridge-specific checks.

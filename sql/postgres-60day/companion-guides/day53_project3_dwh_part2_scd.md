...
5. Make an unchanged-source rerun idempotent.
   **Inputs/evidence:** For sql-53 Exercise 5, read from `dim_customer`, `training.customers`, and `c.country`. Build the answer toward `unchanged_rows_that_would_version`; keep `customer_id` visible whenever the result has row-level grain.
